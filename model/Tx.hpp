...
class Tx
{
    ...
    iroha::protocol::Transaction protobufTransaction;
    CommandGenerator cmdGenerator;

public:
    Tx(const std::string& account_id,
    ...
    Tx& addPeer(const std::string& address,
    ...
    Tx& createAccount(const std::string& account_name,
                      const std::string& domain_id,
                      const std::string& pubkey);
    Tx& createAsset(const std::string& asset_name,
                    const std::string& domain_id,
                    uint32_t precision);
    Tx& createDomain(const std::string& domain_id, const std::string& user_default_role);
    Tx& createRole(const std::string& roleName,
    ...
    Tx& grantPermission(const std::string& account_id,
                        const iroha::protocol::GrantablePermission permission);
    Tx& removePeer(const std::string& pubkey);
    Tx& removeSignatory(const std::string& account_id, const std::string& pubkey);
    Tx& revokePermission(const std::string& account_id,
                         const iroha::protocol::GrantablePermission permission);
    Tx& setAccountDetail(const std::string& account_id,
    ...
    Tx& compareAndSetAccountDetail(const std::string& account_id,
                                   const std::string& key,
                                   const std::string& value,
